...
#ifndef __MASTER_ALLOCATOR_SLAVE_SORTER_RESOURCE_WEIGHTS_SORTER_HPP__
#define __MASTER_ALLOCATOR_SLAVE_SORTER_RESOURCE_WEIGHTS_SORTER_HPP__
...
private:
    ...
    const SlaveID& slaveId, const Resources& resources)
  {
    ...
    if (cpus.isSome()) result += cpuWeight * cpus.get();

    if (gpus.isSome()) result += gpuWeight * gpus.get();
...
#endif // __MASTER_ALLOCATOR_SLAVE_SORTER_RESOURCE_WEIGHTS_SORTER_HPP__
